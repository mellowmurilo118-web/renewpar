      ...
      <div style={{ position: "relative" }}>
        ...
        {isPassword && (
          <button type="button" onClick={() => setShowPwd(!showPwd)} tabIndex={-1}
            style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", padding: 4, color: "#888" }}>
            {showPwd ? (
...
function ErrorAlert({ message, onClose }) {
  ...
  return (
    <div style={{ background: "#fff5f5", border: "1.5px solid #fc8181", borderRadius: 10, padding: "12px 16px", marginBottom: 20, display: "flex", alignItems: "flex-start", gap: 10, animation: "fadeInDown 0.25s ease" }}>
...
function SuccessAlert({ message }) {
  if (!message) return null;
  return (
    <div style={{ background: "#f0fff4", border: "1.5px solid #68d391", borderRadius: 10, padding: "12px 16px", marginBottom: 20, display: "flex", alignItems: "center", gap: 10 }}>
...
export default function Login() {
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      // Admin skips PIN — goes straight to dashboard
  ...
  async function handleResetPassword(e) {
    ...
    try {
      ...
      setShowReset(false);
      // ✅ Stay on login — do NOT navigate away after reset
    } catch (err) {
      setError(friendlyError(err.code));
    } finally {
      setLoading(false);
  ...
  return (
    <>
      ...
      <div style={{ minHeight: "100vh", background: "#f4f6fb", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 16px", fontFamily: "'DM Sans','Trebuchet MS',sans-serif" }}>
        <div style={{ background: "white", borderRadius: 20, padding: "44px 40px", width: "100%", maxWidth: 440, boxShadow: "0 8px 48px rgba(26,26,94,0.10), 0 1px 4px rgba(0,0,0,0.06)", border: "1px solid rgba(26,26,94,0.07)" }}>

          <ShieldLogo />

          <h2 style={{ fontSize: 22, fontWeight: 900, color: "#111", textAlign: "center", marginBottom: 4 }}>Welcome Back</h2>
          <p style={{ textAlign: "center", color: "#888", fontSize: 13.5, marginBottom: 28 }}>Sign in to your Renew Part Bank account</p>
          ...
          {showReset ? (
            <form onSubmit={handleResetPassword}>
              ...
              <Input id="reset-email" label="Email Address" type="email" value={resetEmail}
                onChange={e => setResetEmail(e.target.value)} placeholder="you@example.com" required autoComplete="email" />
              <div style={{ display: "flex", gap: 10 }}>
                <button type="button" onClick={() => { setShowReset(false); setError(""); }}
                  style={{ flex: 1, padding: "12px", borderRadius: 10, border: "1.5px solid #e2e6f0", background: "white", color: "#555", fontWeight: 700, fontSize: 14, cursor: "pointer", fontFamily: "inherit" }}>
                  Cancel
                </button>
                <button type="submit" className="auth-btn"
                  style={{ flex: 2, padding: "12px", borderRadius: 10, border: "none", background: "#1a1a5e", color: "white", fontWeight: 800, fontSize: 14, cursor: "pointer", boxShadow: "0 4px 16px rgba(26,26,94,0.22)", fontFamily: "inherit" }}>
                  Send Reset Link
                </button>
              </div>
            </form>
            ...
            <form onSubmit={handleLogin}>
              <Input id="login-email" label="Email Address" type="email" value={email}
                onChange={e => setEmail(e.target.value)} placeholder="you@example.com" required autoComplete="email" />
              <Input id="login-password" label="Password" type="password" value={password}
              ...
              <div style={{ textAlign: "right", marginTop: -10, marginBottom: 22 }}>
                <span className="auth-link" onClick={() => { setShowReset(true); setError(""); setResetEmail(email); }} style={{ fontSize: 13 }}>
                  Forgot Password?
                </span>
              </div>

              <button type="submit" className="auth-btn"
                style={{ width: "100%", padding: "13px", borderRadius: 12, border: "none", background: "#1a1a5e", color: "white", fontWeight: 900, fontSize: 15, cursor: "pointer", letterSpacing: "0.04em", boxShadow: "0 4px 20px rgba(26,26,94,0.25)", fontFamily: "inherit", marginBottom: 20 }}>
                Sign In
              </button>
            </form>
          )}
          ...
          {/* ✅ Clean Link — no nested <a> inside <span> */}
          <p style={{ textAlign: "center", fontSize: 13.5, color: "#555" }}>
            Don't have an account?{" "}
            <Link to="/register" className="auth-link" style={{ color: "#1a1a5e", fontWeight: 800 }}>
              Create Account
            </Link>
          </p>
