...
//   All other users get Firestore data (or nulls while loading).
//
...
const MOCK_USER = {
  ...
  firstName: "Elliana",
  lastName: "Wilson",
  email: "ellianawilson98@gmail.com",
...
// ─── Null defaults shown while Firestore loads ────────────────────
const NULL_USER = {
...
// ─────────────────────────────────────────────────────────────────
// useDashboardData()
// Waits for onAuthStateChanged — never runs before Firebase is ready.
// Returns authReady=true once auth has resolved so the UI can render.
// ─────────────────────────────────────────────────────────────────
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (firebaseUser) => {
      if (!firebaseUser) {
        ...
        setIsDemo(false); setLoading(false); setAuthReady(true);
        return;
      }
      ...
      if (demo) {
        // ── Demo user: return mock data immediately ─────────────
        setUser(MOCK_USER);
