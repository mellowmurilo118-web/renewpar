...
import { CURRENT_USER, ACCOUNT, TRANSACTIONS, QUICK_ACTIONS, fmt } from "./mockData.js";

// ─── Balance Card ─────────────────────────────────────────────────
function BalanceCard({ onNavigate }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: "linear-gradient(135deg, #1a3cdd 0%, #1a1a9e 40%, #1515cc 100%)" }}>
      ...
      <div className="flex items-center justify-between px-6 pt-5 pb-3">
        <div className="flex items-center gap-3">
          ...
          <div className="flex">
            ...
            <p className="text-white font-bold text-sm">{CURRENT_USER.firstName} {CURRENT_USER.lastName}</p>
            </div>

            <div className="text-white font-extralight text-xs text-right w-20 ml-10">
      ...
      <div className="px-6 pb-4">
        ...
        <p className="text-white font-black text-3xl sm:text-4xl tracking-tight">
          {fmt(ACCOUNT.balance)}
        </p>
      </div>

      {/* BTC row + Action buttons */}
...
function TaxBanner() {
  ...
  return (
    <div className="text-center py-6 relative">
      ...
      <button className="bg-[#1a1a5e] hover:bg-[#2a2a8e] text-white font-bold px-8 py-2.5 rounded-xl text-sm transition-colors shadow-md">
        Tax Refund
      </button>
    </div>
  );
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-4">
        {QUICK_ACTIONS.map(a => (
          <button
            key={a.id}
            onClick={() => onNavigate(routeMap[a.id] || a.id)}
    ...
    <div className="rounded-2xl overflow-hidden border border-blue-200" style={{ background: "#e8eeff" }}>
      <div className="px-5 py-4 border-b border-blue-200">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => onNavigate("transfer-history")}
            className="text-[#1a1a5e] text-xs font-bold hover:underline"
          >
            View all →
          </button>
        </div>
